...
let error =
  QCheck.Test.make ~count:10 ~name:"should_error_raise_exn" QCheck.int (fun _ ->
      raise Error)
...
let stats =
  QCheck.Test.make
    ...
    QCheck.(
      make
        (Gen.int_bound 120)
        ~stats:[ ("mod4", fun i -> i mod 4); ("num", fun i -> i) ])
    (fun _ -> true)
...
let prop_foldleft_foldright =
  let open QCheck in
  Test.make
    ~name:"fold_left fold_right"
    ~count:1000
    ~long_factor:20
    (triple int_gen (list int_gen) (fun2 Observable.int Observable.int int_gen))
    (fun (z, xs, f) ->
...
let prop_foldleft_foldright_uncurry =
  let open QCheck in
  Test.make
    ~name:"fold_left fold_right uncurried"
    ~count:1000
    ~long_factor:20
    (triple (fun1 Observable.(pair int int) int_gen) int_gen (list int_gen))
    (fun (f, z, xs) ->
...
let long_shrink =
  ...
  Test.make ~name:"long_shrink" (pair listgen listgen) (fun (xs, ys) ->
      List.rev (xs @ ys) = List.rev xs @ List.rev ys)

let find_ex =
  let open QCheck in
  Test.make ~name:"find_example" (2 -- 50) (fun n ->
...
let find_ex_uncaught_issue_99 : _ list =
  ...
  in
  let t2 =
    Test.make ~name:"should_succeed_#99_2" ~count:10 int (fun i -> i <= max_int)
  in
  [ t1; t2 ]

(* test shrinking on integers *)
let shrink_int =
  QCheck.Test.make ~count:1000 ~name:"mod3_should_fail" QCheck.int (fun i ->
      i mod 3 <> 0)
...
let stats_tests =
  ...
  [
    Test.make
      ~name:"stat_display_test_1"
      ~count:1000
      (add_stat ("dist", fun x -> x) small_signed_int)
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_2"
      ~count:1000
      (add_stat ("dist", fun x -> x) small_nat)
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_3"
      ~count:1000
      (add_stat ("dist", fun x -> x) (int_range (-43643) 435434))
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_4"
      ~count:1000
      (add_stat ("dist", fun x -> x) (int_range (-40000) 40000))
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_5"
      ~count:1000
      (add_stat ("dist", fun x -> x) (int_range (-4) 4))
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_6"
      ~count:1000
      (add_stat ("dist", fun x -> x) (int_range (-4) 17))
      (fun _ -> true);
    Test.make
      ~name:"stat_display_test_7"
      ~count:100000
      (add_stat ("dist", fun x -> x) int)
      (fun _ -> true);
  ]
