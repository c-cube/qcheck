...
let result_msg = function
  | RSuccess _ -> "Success"
  | RError (_, msg) | RFailure (_, msg) | RSkip (_, msg) | RTodo (_, msg) -> msg
...
let conf_verbose =
  OUnit2.Conf.make_bool "qcheck_verbose" true "enable verbose QCheck tests"

let conf_long =
  OUnit2.Conf.make_bool "qcheck_long" false "enable long QCheck tests"
...
let to_ounit2_test ?(rand = default_rand ()) (QCheck2.Test.Test cell) =
  ...
  let open OUnit2 in
  name
  >: test_case ~length:OUnitTest.Long (fun ctxt ->
         let rand =
           match conf_seed ctxt with
           | -1 -> Random.State.copy rand
           | s ->
         ...
         let print =
           {
             Raw.info = (fun fmt -> logf ctxt `Info fmt);
         ...
         T.check_cell_exn
           ...
           ~long
           ~rand
           ~call:(Raw.callback ~colors:false ~verbose ~print_res:true ~print))
...
let ( >::: ) name l = name >::: List.map (fun t -> to_ounit_test t) l
  ...
  let display_test ?(ended = false) p =
    ...
    let cartouche =
      va
        ...
        (if o = 0 then "" else va " %d!" o)
        total_tests
    and path = string_of_path p in
    let end_marker =
      if cli_args.Raw.cli_print_list then
        if (* print a single line *)
           ended then va " (after %.2fs)\n" (!stop -. !start)
        else "\n"
      else (
        ps Color.reset_line ;
        if ended then " *" else "")
    ...
    let cover =
      if remaining > 0 && not cli_args.Raw.cli_print_list then
